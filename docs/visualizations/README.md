...
| [diffusion-sequence-parallelism-design.html](diffusion-sequence-parallelism-design.html) · [PNG](diffusion-sequence-parallelism-design.png) | vllm-omni diffusion [sequence-parallelism subsystem](https://github.com/vllm-project/vllm-omni/blob/main/docs/user_guide/diffusion/parallelism/sequence_parallel.md) at main `c588208cc` — Ulysses · Ring · Hybrid ×Ring · AllGather-KV | 2026-08-16 | interactive explainer + render; subsystem overview (not tied to one PR); fully offline, zero network deps |
